...
Prebuilt installers: **[GitHub Releases `v2.5.0`](https://github.com/odin-loki/Cypha/releases/tag/v2.5.0)** — `cypha-2.5.0-linux-x86_64.tar.gz`, `cypha-2.5.0-windows-x86_64.zip` (MSVC), `cypha-2.5.0-macos-arm64.tar.gz`, optional **`cypha-2.5.0-linux-x86_64.AppImage`** (Qt shell), **`cypha-2.5.0-arxiv-bundle.zip`** (paper figures).
...
Runs: Release build → **`ctest -R native_`** (authoritative tally: **`scripts/cypha_native_validate_all.ps1`** or `ctest -N -R native_`) → REST contract smoke → bench smoke (d01, d04, d17, forecast) → **`--report-only`** figures → tune dry-run (all smoke configs). Pass **`-TuneSmoke`** for a live **`cypha_tune_run`** sweep.
