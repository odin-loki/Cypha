...
**Author:** Odin Loch
**Status:** Draft — rewritten 2026-07-12; **narrative reconciled 2026-07-17** against canonical D17 hybrid 300k BPC pin **2.873** (`b0d39e7`, [`docs/reports/BASELINE_PIN_CANONICAL_2026-07-17.md`](../docs/archive/reports/BASELINE_PIN_CANONICAL_2026-07-17.md)). Describes the current **native C++** CyphaLM (hybrid GRIA+LSTM, trained by real backpropagation). The Python prototype this paper originally described (`cypha_lm/`) was fully decommissioned in Phase P7 (see `CHANGELOG.md`, "[Unreleased] > Removed": *"Python runtime decommissioned (P7): ... `cypha_lm/` ... removed from the product path. Native C++ ... is the sole runtime."*). Metrics below are sourced from `bench/BASELINE_LOCK.json`, `bench/BASELINE_REPORT.md`, and `docs/RESEARCH_STATUS.md`; every figure/table is annotated with its source, and any claim that could not be re-verified against the native implementation is explicitly flagged rather than carried over from the old draft. **Canonical rule:** cite **2.873** for the locked baseline, production gate, and RPSM stop/go target; other 300k hybrid figures (2.892, 2.864, etc.) are historical run artifacts only.
...
| D17 (WikiText-2, 300k train) | Overnight native run BPC (historical sweep, not lock) | 2.864 | `bench/BASELINE_LOCK.json:overnight_results` |
