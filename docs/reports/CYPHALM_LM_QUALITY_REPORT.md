...
reliable. Those columns were measured when rates were integers floored at 1, so ×0.5 and ×0.25 were nearly the same setting. On lr1_scale 40 shards ×0.5 now really halves every rate and is worse (v3 wiki 1.5954 vs 1.5927 at ×1), so `hp_serve_mixer_lr_scale` defaults to **1**. Set `CYPHA_HP_SERVE_MIXER_LR_SCALE` to opt into a fraction. `CyphaLMModel::set_serve_mode` applies it; generation turns serve mode on, training entry points turn it off.
...
- Weights: linear mixing is worse than geometric (1.7352 vs 1.7248), and
  ...
  on: every scored byte — held-out text in the harness, and the last 512 prompt
  bytes when serving. Generated bytes after the prompt do not move the weights
  unless `learn_from_output` is on. Generation
...
(1), `hp_tree_prune` (1e-4), learned ensemble and ∞-gram weights (adapted
...
- **Mixer rate.** `cyphalm_lm_quality` scores at the *trained* mixer rate
  ...
  (`generate_decode`, REST) runs at 1× (`hp_serve_mixer_lr_scale`, default 1;
  0.5 is opt-in). Every
  held-out NLL in this report is at 1× unless it says ×0.5 (the serve-rate
  table, `aq_2_*`, `val_2_*`). The 0.5× gain in that table was measured when
  the scale floored at rate 1.
...
| `hp_serve_mixer_lr_scale` | 1 | `CYPHA_HP_SERVE_MIXER_LR_SCALE` | serve mixer rate, applied by `set_serve_mode`; resolution 1/16 of each positive rate (a trained 0 stays 0). 0.5 is worse on lr1_scale 40 shards |
...
override the values stored in a loaded checkpoint. After the `.hpbin` is
read, those drops and caps are applied again: the file used to turn dropped
models back on, and pool slots at or above `pool_slots` predicted but never
learned.
