...
param(
    ...
    [ValidateSet("d17", "d21", "cell-sweep")]
    [string]$Run,
...
Write-Host "== baseline lock update (run=$Run n_train=$(if ($NTrain -gt 0) { $NTrain } elseif ($Fast) { '200 (fast default)' } else { '300000' }) fast=$Fast) ==" -ForegroundColor Cyan
& $exe @args
if ($LASTEXITCODE -ne 0) {
    throw "cypha_baseline_lock failed exit=$LASTEXITCODE"
}
