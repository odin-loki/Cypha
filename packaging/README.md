...
pwsh -File scripts/create_release_notes.ps1 -Tag v2.3.25
```
...
Prebuilt installers: **[GitHub Releases — latest (`v2.3.25`)](https://github.com/odin-loki/Cypha/releases/tag/v2.3.25)**.

```bash
# Linux CLI tarball
tar xzf cypha-2.3.25-linux-x86_64.tar.gz && cd cypha-2.3.25-linux-x86_64 && bash install.sh

# Linux Qt AppImage (no install step)
chmod +x cypha-2.3.25-linux-x86_64.AppImage
./cypha-2.3.25-linux-x86_64.AppImage
```

```powershell
# Windows CLI zip (MSVC)
Expand-Archive cypha-2.3.25-windows-x86_64.zip
cd cypha-2.3.25-windows-x86_64
...
```bash
# From repo root
bash packaging/build_appimage.sh 2.3.25

# Re-package from an existing CMake tree (e.g. CI build-release):
SKIP_BUILD=1 bash packaging/build_appimage.sh 2.3.25 native/build-release dist
```
...
```bash
chmod +x dist/cypha-2.3.25-linux-x86_64.AppImage
./dist/cypha-2.3.25-linux-x86_64.AppImage

# Headless smoke (demo fixtures bundled when built from a full tree)
./dist/cypha-2.3.25-linux-x86_64.AppImage --appimage-extract-and-run --smoke \
...
```powershell
dist\cypha-2.3.25-windows-qt\cypha_qt_shell.exe
dist\cypha-2.3.25-windows-qt\cypha_qt_shell.exe --smoke share\demo_fixtures\reference.cypha
```
