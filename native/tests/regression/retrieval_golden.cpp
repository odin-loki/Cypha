// retrieval_golden — CyphaDIF.retrieve vs native retrieve_from_x.
// Usage: retrieval_golden <fixtures/retrieval/sidecar.json>
#include <cmath>
#include <fstream>
#include <iostream>
#include <sstream>
...
int main(int argc, char** argv) {
  try {
    if (argc != 2) {
      std::cerr << "usage: retrieval_golden <fixture_dir>/sidecar.json\n";
      return 2;
    }
    ...
    if (failures > 0) {
      std::cerr << "retrieval_golden: " << failures << " case(s) failed\n";
      return 1;
    }
    std::cout << "retrieval_golden OK\n";
    return 0;
  } catch (const std::exception& e) {
    std::cerr << "retrieval_golden error: " << e.what() << "\n";
    return 1;
  }
}
