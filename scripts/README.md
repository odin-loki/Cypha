...
| `verify_production_pipeline.ps1` | Phase 21 (shipped): unified maintainer smoke - production complete + release publish + repo smoke cleanup preview + optional d35 (`-AllowPending` when lock below 300k); default tag `v2.3.24` or `git describe` | console |
...
pwsh -File scripts\verify_release_publish.ps1 -BuildDir native\build -Tag v2.3.24
...
gh auth login
gh auth status
pwsh -File scripts\publish_release.ps1 -Tag v2.3.24
...
| **`verify_production_pipeline.ps1`** | Default release tag **`v2.3.24`** (or **`git describe --tags --abbrev=0`** when available) | manual |
