...
### Added
- **Competition housekeeping:** root [`LICENSE`](LICENSE) (CC BY 4.0) and [`MODEL_CARD.md`](MODEL_CARD.md); living docs aligned to 2.664 BPC / 36 cell-sweep variants. Closed BoW / optimality / forecasting design moved under `docs/archive/`; root BoW is open items only. Forecast/codec sources rewritten UTF-8 (OneDrive UTF-16 broke GCC/Linux CI); `.gitattributes` pins `working-tree-encoding=UTF-8`. Historical XOR/D17 traces moved from `artifacts/profiles/` to [`data/archive/`](data/README.md); repo-root `d*_smoke.json` leaks removed.
