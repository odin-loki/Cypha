...
CI runs **four jobs** (`.github/workflows/ci.yml`): **`build_and_test`** (Ubuntu native build + CTest), **`mingw_cross`** (MinGW Windows PE cross-build), **`windows_cuda_msvc`** (MSVC + CUDA compile smoke), and **`linux_cuda`** (GCC + nvcc compile smoke). The Linux **`build_and_test`** job gates REST smokes via **`CYPHA_REST_BIN`**. Both CUDA jobs exercise **`native_cuda_smoke`** / **`native_score_batch`** on CPU threads when the runner has no GPU.
