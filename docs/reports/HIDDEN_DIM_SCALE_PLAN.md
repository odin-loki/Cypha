...
### 4. Production-scale (`n_train=300000`) re-confirmation: **in progress, not yet complete at time of writing**
...
Both were started and, as of this writing, are still on their first attempt with no `-1`/empty-output failures observed yet. This machine was under heavy, directly-observed contention throughout this pass (simple shell commands like directory listings routinely took 20 seconds to several minutes to return — consistent with this document's own prior observation of "15–18 concurrent `cyphalm_bench_native.exe`/other native-benchmark processes" from sibling agents), so both runs should be expected to land at or above the high end of the previously observed wall-clock range (hidden=128 clean ≈52 min; hidden=512 previously measured at 16h39min under comparable contention) rather than the uncontended §5.2 estimate.

**This document is being committed with the production re-confirmation explicitly incomplete, per this task's own instruction not to guess or extrapolate a result not actually measured.** The mechanism fix itself (§1–2) and the control-sweep confirmation (§3) are complete and are not contingent on the production run finishing — they establish *that* the sampling bug existed and *that* the fix addresses it at the mechanism level (uniform `sample_ratio=2.0` now achieved at every `lstm_hidden`, where before it ranged from 0.375 down to 0.094 and got worse precisely as `lstm_hidden` grew). What remains open is whether, once the underdetermined-covariance artifact is removed, `lstm_hidden_d_eff` at production scale (300k steps) actually recovers the expected upward 128→512 trend (supporting the "was purely a sampling artifact" explanation) or still fails to climb (which would instead support Finding 2's second, non-exclusive explanation — genuine representational compression at scale, an authentic finding about training dynamics rather than a measurement bug). **Do not treat either outcome as already known** — check `bench/results/hidden_dim_scale_deff/300k_hidden128.json` and `300k_hidden512.json` (and their `*_watchdog.log` siblings) for the actual completed numbers before drawing that conclusion; if this document has not been updated with a result table in this section by the time those files exist, treat the re-confirmation as still-pending follow-up work.

### 5. Updated go/no-go recommendation on Phase 4 (1024-dim)

**Still no-go, pending the production re-confirmation in §4 — upgraded from "no-go, sampling gap unaddressed" to "no-go, sampling gap addressed but awaiting the actual re-measurement it was meant to enable."** The original Phase 3 no-go had three legs (§ above): (1) the `lstm_hidden_d_eff` inversion itself, (2) wall-clock cost far above estimate, (3) BPC/kappa alone being weaker, non-`D_eff`-specific evidence. This pass resolves the *measurement* half of leg (1) — the statistic is no longer known to be underpowered — but does not yet resolve the *empirical* half: whether the underlying trend, once measurable without the sampling artifact, actually climbs at production scale. Legs (2) and (3) are unchanged by this pass. **Do not schedule Phase 4 until the §4 production re-confirmation completes and is recorded here with an explicit result table** (mirroring §"Result table" under the original Phase 3 section) — if `lstm_hidden_d_eff` climbs 128→512 post-fix at 300k steps, that would resolve the specific concern this plan is built around and make a Phase 4 go-decision worth revisiting on its own (unchanged) cost/contention merits; if it still does not climb, that is itself a significant, novel finding (genuine width-independent representational compression at scale) that would argue against Phase 4 even more strongly than the original inversion did, since it would no longer be attributable to a measurement bug.
