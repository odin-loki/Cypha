...
| **`cypha_baseline_lock`** | Merge D17 / d21 / cell-sweep BPC into **`bench/BASELINE_LOCK.json`**; **`--run all`** chains all three lock runs; **`--medium`** for 5k train tier (`status=medium_smoke`). CTest **`native_baseline_lock_smoke`**. Wrapper: **`scripts/update_baseline_lock.ps1`**. Validator: **`scripts/validate_baseline_lock.ps1`**, **`baseline_lock_validate`**. |
...
| **`baseline_lock_validate`** *(Phase 12)* | Validate **`bench/BASELINE_LOCK.json`** schema + d17 hybrid pin. CTest **`native_baseline_lock_validate_smoke`**. PS wrapper: **`scripts/validate_baseline_lock.ps1`**. |
