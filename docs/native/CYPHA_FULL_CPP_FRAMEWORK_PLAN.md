...
**Status:** **v2.5 complete — SHIPPED** ([`v2.5.0` release](https://github.com/odin-loki/Cypha/releases/tag/v2.5.0) — hp CyphaLM + Hybrid 2.664 BPC lock + forecasting; Linux + Windows MSVC + **macOS arm64** installers, AppImage, arxiv bundle; **CI fully green** — two blocking jobs: **`build_and_test`**, **`windows_msvc`**).
...
GitHub Actions **CI**: Linux **`build_and_test`** + **`windows_msvc`** (MSVC Release). MinGW is optional local only — not a CI/release gate. CUDA jobs removed — validate locally. Release **v2.5.0** installers published (Linux + Windows + macOS arm64).
...
| **`v2.5.0`** tag | **Latest release** — hp CyphaLM + Hybrid 2.664 BPC + forecasting; Linux/Windows/macOS arm64; two CI jobs blocking (`build_and_test`, `windows_msvc`); CUDA local-only |
...
git push origin main --tags   # publish release (see v2.5.0 on GitHub Releases)
```
