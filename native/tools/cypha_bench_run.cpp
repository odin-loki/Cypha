...
Json train_eval_vectors(const std::vector<std::vector<double>>& train_x, const std::vector<std::string>& train_y,
    ...
    const int train_n = static_cast<int>(train_x.size());
    ...
    for (int p = 0; p < passes; ++p) {
        ...
        for (int idx : order) {
            cypha::dif_train_step_vector(infer, mem, replay, train_x[static_cast<std::size_t>(idx)].data(), d,
    ...
    y_true.reserve(test_x.size());
    y_pred.reserve(test_x.size());
    for (std::size_t i = 0; i < test_x.size(); ++i) {
        y_true.push_back(test_y[i]);
        std::vector<double> llr;
        cypha::batch_llr_from_x(infer, test_x[i].data(), 1, llr);
        int best = 0;
    ...
    Json result = Json{
        ...
        {"baselines", cypha::bench::offline_classification_baselines_json(train_x, train_y, test_x, test_y)},
        {"n_train", train_n},
        {"n_test", static_cast<int>(test_x.size())},
...
int main(int argc, char** argv) {
    try {
        ...
        if (!args.domain_tag.empty()) {
            ...
            selected.clear();
            for (const auto& d : domains) {
                if (domain_number(d.tag) >= args.from_domain) selected.push_back(d);
            }
        }
