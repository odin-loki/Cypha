...
- **Intelligence Stats Phase 24 (v2.3.24) — prep:** bench **d38** production overnight completion certificate — full 300k overnight completion gate (overnight/RPSM/cell-sweep alignment, 28-variant sweep); profile **`bench/config/d38_overnight_certificate_profile.json`**; **`scripts/poll_and_finalize_overnight.ps1 -AutoCommit`** (post-finalize lock commit when **`n_train >= 300000`**); **`scripts/watch_production_overnight.ps1`** stall detector (**`-StallMinutes`**, **`-LogFile`**); **`CYPHA_VALIDATE_OVERNIGHT_CERTIFICATE=1`**; CTest **`native_d38_overnight_certificate_smoke`** *(when merged)* (**116 total**).
