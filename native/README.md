...
| **`d27_production_lock_smoke`** *(Phase 13)* | Bench **d27** production overnight lock validation (`cypha_bench_run --domain-tag d27`); if **`overnight_results.n_train < 300000`**, reports **`pending_production`** (smoke pass); if **≥ 300k**, validates BPC within **0.05** of the **historical** d17 hybrid **2.873** pin. CTest **`native_d27_production_lock_smoke`**. |
