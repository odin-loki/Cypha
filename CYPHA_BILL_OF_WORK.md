...
- [ ] **Numerical audit — 3 reachable defects left open, 13 latent; 1 fixed** — full report: [`docs/reports/NUMERICAL_KERNEL_AUDIT_2026-09-17.md`](docs/reports/NUMERICAL_KERNEL_AUDIT_2026-09-17.md). **R1 is fixed**: `classify_at_h` applied `world_gate` twice on the kernel-LLR path (`confidence = disc·gate²`), measured as a 45.8% understatement of confidence on an identical label; removed and guarded by CTest `native_kernel_gate_invariant`. Safe to change because the kernel path has no Python ancestor and `PORT_CONTRACT.md:65` excludes it from parity fixtures. **Still open, and deliberately not fixed** — all three are faithful ports whose Python originals are cited in the report, so diverging is a port-contract decision: **R2** `infer_cpu.cpp:1271-1280` builds the anomaly score by dividing a latent-variance-scaled `r_eff` by a dimensionless `mahal_ema`, on the default `/predict` path (`use_gh` defaults true), scoring 0.0 where the fixture should give 0.18–1.44; **R3** `infer_cpu.cpp:865-885` drops confidence exactly `1.0` from ECE binning and scores an all-NaN evaluation as a perfect 0.0 (Python `Cypha.py:137`); **R4** `infer_cpu.cpp:903-950` never scores the incumbent temperature, so `/adapt_temperature` can install a worse one (Python `Cypha.py:3129`). A parity-preserving improvement for R4 would be to return the chosen ECE alongside `T` so callers can reject a regression.
