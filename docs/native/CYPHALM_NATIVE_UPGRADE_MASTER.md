...
1. **Hybrid parity** — ? **2.892 BPC @ 300k** (Python 2.873, ? +0.019); root cause was in-place LSTM `forward_step`
...
- [x] REST LM smoke: `native/scripts/smoke_cyphalm_rest_lm.py`; ctest `tests/test_cyphalm_rest_lm_smoke.py`
- [x] Checkpoint DIF + SSM state save/load (`NIGExpert::state_dict`, `CyphaDIF::get_state`, `CellAISSM::get_state`)
- [x] Parity runner repo-root sidecar resolution; ctest fixture key fix
- [x] `scripts/cyphalm_native_validate.ps1` (build + CTest + ctest + REST smoke)
