...
| **`baseline_lock_validate`** *(Phase 12–13)* | Validate **`bench/BASELINE_LOCK.json`** schema + d17 hybrid pin; **`--production`** enforces production/completed status @ 300k. CTest **`native_baseline_lock_validate_smoke`**. PS wrapper: **`scripts/validate_baseline_lock.ps1`** (`-Strict`, **`-Production`**). |
