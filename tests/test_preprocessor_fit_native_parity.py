"""
``preprocessor_fit_parity`` — native ``PreprocessorState::fit_from_design_matrix`` (scale on/off + PCA + RFF).

Runs ``preprocessor_fit/``, ``preprocessor_fit_no_scale/``, and ``preprocessor_fit_rff/``.
CTest: ``native_preprocessor_fit``.
...
_FIX_RFF = _ROOT / "parity_fixtures" / "preprocessor_fit_rff"


def test_preprocessor_fit_parity_subprocess():
    for base in (_FIX, _FIX_NS, _FIX_RFF):
