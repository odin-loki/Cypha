...
bool use_bit_tree_byte_log_probs() {
    const char* v = std::getenv("CYPHA_HP_BIT_TREE_LOGPROBS");
...
void HpSequenceBackend::expand_bit_tree_dfs(const hp::Config& cfg, int vocab_size, int depth,
                                            int prefix, double log_p_nats, hp::Predictor& node,
    ...
    for (int bit = 0; bit <= 1; ++bit) {
        ...
        hp::Predictor child = hp::Predictor::clone_from(node, cfg);
        const double child_log = log_p_nats + bit_log_prob(p12, bit);
        child.update(bit);
        const int next_prefix = (prefix << 1) | bit;
        expand_bit_tree_dfs(cfg, vocab_size, depth + 1, next_prefix, child_log, child,
                            out_log_nats);
    }
}
...
std::vector<double> HpSequenceBackend::next_byte_log_probs_bit_tree(int vocab_size) const {
    ...
    hp::Predictor root = hp::Predictor::clone_from(*pred_, cfg_);
    expand_bit_tree_dfs(cfg_, n, 0, 0, 0.0, root, out);
    return out;
}
...
std::vector<double> HpSequenceBackend::next_byte_log_probs(int vocab_size) const {
    if (use_bit_tree_byte_log_probs()) {
...
double HpSequenceBackend::log_prob_byte(std::uint8_t byte) const {
    hp::Predictor snap = hp::Predictor::clone_from(*pred_, cfg_);
    return byte_log_prob(snap, static_cast<int>(byte));
}
