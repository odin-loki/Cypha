// cyphalm_model_golden — 10-token forward-pass scaffold for Tier 2 CyphaLM native.
// Usage:
...
int run_scaffold(ContextMode mode, const std::string& json_path) {
    ...
    const auto schedule = cypha::cyphalm::resolve_view_schedule_struct("schedule_a", 42, 1);
    const auto view_epochs =
        cypha::cyphalm::iter_view_epochs(std::vector<int>(tokens.begin(), tokens.end()), schedule);
    std::cout << "cyphalm_model_golden: mode=" << cypha::cyphalm::context_mode_name(mode)
              << " vocab=" << model.config().vocab_size << " steps=" << tokens.size()
              << " view_epochs=" << view_epochs.size() << "\n";
    ...
    std::cout << "alpha_mean=" << snap.mean_alpha << " edge_frac=" << snap.fraction_near_edge_of_chaos
              << "\n";
    std::cout << "OK cyphalm_model_golden scaffold (" << tokens.size() << " tokens)\n";
    return 0;
}
...
int main(int argc, char** argv) {
    try {
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            } else if (arg == "--smoke-ablations") {
                smoke_ablations = true;
            } else if (arg == "--help" || arg == "-h") {
                std::cout
                    << "cyphalm_model_golden [checkpoint.json] [--mode "
                       "full|gria_ngram|hybrid|char_lstm|ssm_gria|ssm_gria_no_lstm|ablation_no_dif|"
                       "ablation_no_ssm] [--smoke-ablations]\n";
                return 0;
            } else if (arg[0] != '-') {
                json_path = arg;
            }
        ...
        if (smoke_ablations && json_path.empty()) {
            int rc = 0;
            rc |= run_scaffold(ContextMode::AblationNoDif, "");
            rc |= run_scaffold(ContextMode::AblationNoSsm, "");
            return rc;
        }

        return run_scaffold(mode, json_path);
    } catch (const std::exception& ex) {
        std::cerr << "FAIL cyphalm_model_golden: " << ex.what() << "\n";
        return 1;
    }
}
