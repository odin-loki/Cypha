...
Monorepo C++ core for **One Cypha** -- public type **`cypha::Cypha`** (classify + regress + latent sample + sequence tokens). Port contract: [`docs/port/PORT_CONTRACT.md`](../docs/port/PORT_CONTRACT.md). Full-stack milestones: [`docs/port/PORT_FULL_STACK.md`](../docs/port/PORT_FULL_STACK.md). Cutover: [`docs/reports/ONE_CYPHA_CUTOVER.md`](../docs/reports/ONE_CYPHA_CUTOVER.md).

**Vendored:** `third_party/nlohmann/json.hpp`, `third_party/httplib.h` (OpenSSL is optional -- see **Federated TLS** below).

**Sequence / CyphaLM (hp gate24):** living byte **LLM** — train (`train_step`, BPC eval) + serve (`serve_predict_next`, `generate_decode`, `cyphalm_generate`). **Inference latency first.** gate24 only (v78 + `HP_SLOT_MAX=24`). Quality bar **~1.61 BPC** enwik observe. Legacy Hybrid GRIA+LSTM not linked by default — [`docs/history/LEGACY_LLM.md`](../docs/history/LEGACY_LLM.md). Serve API: [`docs/native/CYPHALM_SERVE.md`](../docs/native/CYPHALM_SERVE.md).
...
| **`cypha_lm_native`** | INTERFACE alias of `cypha_core` (sequence sources compiled into core). Build notes: [`CYPHALM_NATIVE_BUILD.md`](../docs/native/CYPHALM_NATIVE_BUILD.md) |
...
| **`cyphalm_bench_native`** | Per-profile sequence BPC bench (PGM / char-LSTM / **historical hybrid** / ablations). Used by **`cypha_bench_run`** d04/d17 and **`cypha_tune_run`**. |
...
| **`d27_production_lock_smoke`** *(Phase 13)* | Bench **d27** production overnight lock validation (`cypha_bench_run --domain-tag d27`); if **`overnight_results.n_train < 300000`**, reports **`pending_production`** (smoke pass); if **≥ 300k**, validates BPC within **0.05** of the d17 hybrid **2.664** pin (`d17_hybrid_baseline.bpc`). CTest **`native_d27_production_lock_smoke`**. |
| **`d28_overnight_complete`** *(Phase 14)* | Bench **d28** unified overnight completion validation (`cypha_bench_run --domain-tag d28`); validates schema + production tier + matching **`n_train`** / **`n_eval`** across **`overnight_results`**, **`rpsm_results`** (d21 hp; legacy lock key), and **`cell_sweep_results`**; **`pending_overnight_complete`** when **< 300k** (smoke pass). CTest **`native_d28_overnight_complete_smoke`**. |
| **`baseline_lock_validate`** *(Phase 12–14)* | Validate **`bench/BASELINE_LOCK.json`** schema + d17 hybrid pin from lock; accepts **`medium_smoke`** / **`production`** statuses; **`--production`** enforces production/completed status @ 300k. CTest **`native_baseline_lock_validate_smoke`**. PS wrapper: **`scripts/validate_baseline_lock.ps1`** (`-Strict`, **`-Production`**). Post-overnight: **`scripts/finalize_production_overnight.ps1`**. |
...
**Overnight runners (maintainer):** **`scripts/run_d17_overnight.ps1`**; Phase 9 unified **`scripts/run_overnight_all.ps1`** chains D17 + d21 hp lock + cell sweep + baseline-lock refresh (RPSM retired — [`docs/history/REMOVED_RPSM.md`](../docs/history/REMOVED_RPSM.md)). Phase 10 adds **`cypha_baseline_lock --run all`** and Windows TLS mirror **`scripts/ci_federated_tls_windows.ps1`**. Phase 11 adds WikiText download (**`scripts/download_wikitext2.ps1`**, **`scripts/download_wikitext2.sh`**), gutenberg fallback for d17/d21, **`corpus_smoke`**, bench **d25**, and **`-Fast`** propagation on overnight scripts. Phase 12 adds **`-Medium`** tier (5k train, real corpus), bench **d26**, **`validate_baseline_lock.ps1`** / **`baseline_lock_validate`**, and **`publish_release.ps1 -DryRun`** *(shipped everywhere)*. Phase 13 adds **`-Production`** tier (300k train / 2000 eval), dedicated **`scripts/run_production_overnight.ps1`**, bench **d27**, and **`validate_baseline_lock.ps1 -Production`**. Phase 14 adds status validator fix (**`medium_smoke`** / **`production`**), cell-sweep output **`bench/results/cell_sweep`**, bench **d28**, and **`finalize_production_overnight.ps1`**. Phase 15-24 add release readiness (**d29**), artifact hygiene (**d30**), post-overnight pipeline (**d31**), production complete (**d32**), release publish (**d33**), repo smoke hygiene (**d34**), lock commit pipeline (**d35**), production pipeline E2E (**d36**), overnight lock refresh (**d37**, shipped), and overnight completion certificate (**d38**, shipped) gates. See [`docs/reports/INTELLIGENCE_STATS_IMPLEMENTATION.md`](../docs/archive/reports/INTELLIGENCE_STATS_IMPLEMENTATION.md) Phase 9-24.
...
**Qt:** [`cmake -DCYPHA_BUILD_QT=ON`](qt/README.md) builds **`cypha_qt_stub`** (Core) and **`cypha_qt_shell`** (Widgets). Optional **`-DCYPHA_QT_CHARTS=ON`** links Qt Charts for the shell loss widget when **`Qt6::Charts`** is installed. **GitHub CI:** two **blocking** jobs — **`build_and_test`** installs **`qt6-base-dev`** (Charts off), passes **`-DCYPHA_BUILD_QT=ON`**, and runs **`native_qt_stub_load_reference`**; headless Linux excludes GUI exec tests **`native_qt_shell_smoke`**. **`windows_msvc`** verifies native MSVC Release artifacts. Optional third job **`federated_tls`** (**`-DCYPHA_ENABLE_OPENSSL=ON`**, **`native_federated_tls_smoke`**, `continue-on-error`; local mirrors **`bash scripts/ci_federated_tls_linux.sh`**, **`scripts/ci_federated_tls_windows.ps1`** *(Phase 10)*). Optional fourth job **`corpus_and_d25`** (WikiText fetch + **`native_corpus_smoke`** / **`native_d25_corpus_smoke`**, `continue-on-error`; Phase 12). **~214 CTests** in the blocking gate (`ctest -R native_`; see **`scripts/cypha_native_validate_all.ps1`** for the authoritative tally). Production overnight (300k) is **not** run in CI — use **`scripts/run_production_overnight.ps1`** (+ **`finalize_production_overnight.ps1`**, **`validate_production_complete.ps1`**, **`verify_release_publish.ps1`**, **`verify_production_pipeline.ps1`**, **`run_post_overnight.ps1`**) locally. CUDA is not in CI — build locally with **`-DCYPHA_ENABLE_CUDA=ON`** and run **`native_cuda_smoke`** / **`native_score_batch`** (see [`ACCEL_CUDA.md`](../docs/native/ACCEL_CUDA.md)). Local **`scripts/ci_native_linux.sh`** defaults Qt OFF unless **`CYPHA_BUILD_QT=1`**; optional **`CYPHA_QT_CHARTS=1`** passes **`-DCYPHA_QT_CHARTS=ON`** (install **`qt6-charts-dev`** first).
