...
$cleanupScript = Join-Path $PSScriptRoot "cleanup_repo_smoke_artifacts.ps1"
...
if (-not $useAllowPending) {
    ...
    if ($nTrain -lt $PRODUCTION_N_TRAIN_MIN) {
        ...
        Write-Host "verify_production_pipeline: lock n_train=$nTrain below $PRODUCTION_N_TRAIN_MIN - auto -AllowPending" -ForegroundColor Yellow
    }
}

Write-Host "== verify production pipeline (smoke) ==" -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "verify_production_pipeline: FAIL (validate_production_complete)" -ForegroundColor Red
    exit $LASTEXITCODE
}
...
if ($benchExe -and (Test-DomainTagExists -Tag "d33")) {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            Write-Host "verify_production_pipeline: FAIL (d33 bench)" -ForegroundColor Red
            exit $LASTEXITCODE
        }
    } finally {
        Pop-Location
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "verify_production_pipeline: FAIL (publish_release -DryRun)" -ForegroundColor Red
    exit $LASTEXITCODE
}

Write-Host ""
Write-Host "== cleanup_repo_smoke_artifacts.ps1 -DryRun ==" -ForegroundColor Cyan
& $cleanupScript -DryRun
if ($LASTEXITCODE -ne 0) {
    Write-Host "verify_production_pipeline: FAIL (cleanup_repo_smoke_artifacts -DryRun)" -ForegroundColor Red
    exit $LASTEXITCODE
}

if ($benchExe -and (Test-DomainTagExists -Tag "d35")) {
    Write-Host ""
    Write-Host "== cypha_bench_run --domain-tag d35 ==" -ForegroundColor Cyan
    Push-Location $root
    try {
        & $benchExe --domain-tag d35
        if ($LASTEXITCODE -ne 0) {
            Write-Host "verify_production_pipeline: FAIL (d35 bench)" -ForegroundColor Red
            exit $LASTEXITCODE
        }
    } finally {
        Pop-Location
    ...
    Write-Host ""
    Write-Host "== cypha_bench_run --domain-tag d35 (skipped - exe not found under $BuildDir) ==" -ForegroundColor DarkGray
} else {
    Write-Host ""
    Write-Host "== cypha_bench_run --domain-tag d35 (skipped - domain not present) ==" -ForegroundColor DarkGray
}

Write-Host ""
Write-Host "verify_production_pipeline: OK - smoke checks passed" -ForegroundColor Green
