...
| **Qt** (`cypha_qt_stub`, **`cypha_qt_shell`**) | **`sudo apt-get install qt6-base-dev`** (Debian/Ubuntu), then **`cmake … -DCYPHA_BUILD_QT=ON`** — CTests **`native_qt_stub_load_reference`**, **`native_qt_shell_smoke`**; pytest **`tests/test_qt_stub_native.py`**, **`tests/test_qt_shell_native.py`**. Local: **`CYPHA_BUILD_QT=1 bash scripts/ci_native_linux.sh`**. Headless CI **compiles** Qt but **excludes** GUI exec CTests. |
| **CyphaLM checkpoint sidecars** | `python scripts/generate_cyphalm_checkpoint_fixture.py` and `generate_cyphalm_hybrid_checkpoint_fixture.py` → `parity_fixtures/cyphalm_checkpoint/*/sidecar.json` with **`checkpoint_json": "checkpoint.json"`** (relative to sidecar dir; portable on Linux CI). CTest **`native_cyphalm_checkpoint_parity`**. |
| **CyphaLM component fixtures** | `python scripts/generate_cyphalm_native_fixtures.py` (char_lstm, ssm, …). |
