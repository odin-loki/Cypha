...
- `preprocessor/` — `preprocessor.json` + `sidecar.json` for `preprocessor_parity` (from main generator / `generate_parity_fixtures.py`)
...
- `two_stage_pipeline/sidecar.json` — `two_stage_dif_predict_with_clf` LLR + RFF stage-2 (`generate_two_stage_pipeline_fixture.py`)
- `two_stage_ridge_fit/sidecar.json` — ridge fit from LLR (`generate_two_stage_ridge_fit_fixture.py`)
- `two_stage_e2e_ridge/sidecar.json` — quantile-DIF LLR → ridge end-to-end (`generate_two_stage_e2e_ridge_fixture.py`)
