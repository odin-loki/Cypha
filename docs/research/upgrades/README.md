...
Distilled specs backported from the former `Cypha Possible Upgrades/` folder (2026-06). These are **historical research specs** unless a row below says otherwise. **RPSM and cell-sweep tracks are STOP/closed** per BoW (2026-07-17). Living sequence work is **Hybrid ~2.8 BPC + predictive arithmetic coding**.
...
| [RPSM_IMPLEMENTATION.md](RPSM_IMPLEMENTATION.md) | RPSM core dynamics, five critical fixes (spectral ?, normalised ?, orthogonal init, multi-level injection, symmetric W_down) | **STOP** ? archived implementation detail |
| [NONLINEAR_BOUNDARY.md](NONLINEAR_BOUNDARY.md) | Nonlinear discriminant fixes (Nystr?m, RFF, MLP encoder, GRIA-? kernel, spectral mixture) | Partially shipped (Nystr?m kernel LLR); remainder historical |
...
| [ARCHITECTURE_BPC_LEARNINGS_2026-07-19.md](ARCHITECTURE_BPC_LEARNINGS_2026-07-19.md) | Mixer + depth path toward ?2.0 BPC | **Living** ? architecture notes |
