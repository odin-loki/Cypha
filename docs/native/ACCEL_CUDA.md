...
**GitHub Actions (optional job):** **`ilammy/msvc-dev-cmd`** + **Ninja** single-config (`cmake -G Ninja -DCYPHA_ENABLE_CUDA=ON`); configure/build/ctest run in one step so MSVC env persists. See [`.github/workflows/ci.yml`](../../.github/workflows/ci.yml) job `windows_cuda_msvc`.
