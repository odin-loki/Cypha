...
Cypha's native port (M1�M6 + P7) is complete � inference, training, REST server, Qt shell, experiments DB, and parity fixtures all pass CI (**106 CTests**). Python runtime packages removed. This document records the most valuable next engineering directions, from near-term (months) to longer-horizon (quarters).
...
| pytest CI gate (~274 tests) | **Removed** � **106 CTests** (`ctest -R native_`) gate releases |
